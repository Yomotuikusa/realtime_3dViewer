...
- realtime-dispatch.ts: `ServerMessage` を session / presence / annotation / comments / lighting / objects ストアへ振り分ける入口。`welcome` の共有ライト・非表示版、presence 更新（camera の焦点距離を含む）、light、stroke、`comment:created` / `comment:updated`、オブジェクト可視性・追加、`error` を扱い、未知の型はコンパイル時に検出する
- useRealtime.ts: 名前決定後の `WsClient` 接続と、open ごとの `join` 送信。`onRealtimeStatus` は session の接続状態を更新し、open 時に lastError を解除する
- review-stores.ts: レビュー画面のアンマウント時に session / presence / annotation / comments / camera / lighting / objects / playback の8ストアをまとめて初期化する reset 関数（shortcuts ストアは対象外）
...
`App` は projectId を `ReviewPage` の React key に使い、プロジェクト切替時のアンマウントで `resetReviewStores()` を実行して8ストアを初期化する。`ReviewPage` は ready/error に取得対象の `projectId` を保持し、現在の URL と一致しない間は
...
`dispatchServerMessage` は `welcome` で session の self ID/色、presence の参加者一覧、annotation のライブ線、任意の共有ライト、非表示オブジェクト一覧を確定し、
`user:joined` / `user:left` / `camera` を presence ストアへ、`stroke:add` / `stroke:remove` / `stroke:clear` を
annotation ストアへ、`light` を lighting ストアへ、`comment:created` / `comment:updated` を comments ストアへ、`object:visibility` / `object:added` を objects ストアへ反映し、`error` を `CODE: message` として保存する。

## テスト
- tests/display-name.test.ts: 表示名の trim、保存、Guest 名、localStorage 例外のテスト
- tests/realtime-dispatch.test.ts: welcome の session / presence / annotation / light / objects 反映、焦点距離を含む presence/stroke/comment イベント、object 追加・可視性、error、未対応イベント、reset のテスト
- tests/review-labels.test.ts: 接続状態・コピー状態・ロード/エラー文言のテスト
- tests/review-stores.test.ts: 8つのレビュー用ストアをまとめて初期化する reset の検証
