...
## 目的
レビュー画面のカメラ、照明、再生、セッション、presence、annotation、comments、ショートカットの状態を zustand ストアで保持する。
...
- playback.ts: ロード中モデルのアニメーションクリップ要約、選択、再生／一時停止、時刻シークを保持する zustand ストア
...
playback ストアは `clips` を要素ごとに複製して保持し、クリップ選択時に時刻を0へ戻す。レビュー画面の reset では clips、選択、再生状態、時刻を初期値へ戻す。
