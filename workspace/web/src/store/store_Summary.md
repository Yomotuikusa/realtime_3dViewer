...
- display.ts: ルーム共有メッシュ表示方法、メッシュ比較設定、ジョイント表示設定を複製保持し、同値更新を抑止する `useDisplayStore` を提供する
...
- display.ts: `useDisplayStore`、`DisplayStoreState`（`meshDisplay` / `meshCompare` / `jointDisplay` と各 setter、reset）
...
display ストアは `MeshDisplayMode`、`MeshCompare`、`JointDisplay` を保持し、初期値と reset はそれぞれ `DEFAULT_MESH_DISPLAY` / `DEFAULT_MESH_COMPARE` / `DEFAULT_JOINT_DISPLAY` の複製、同値の set は state と購読通知を変えない。比較設定とジョイント表示設定の setter、welcome 受信値は浅く複製して保持する。
...
- tests/store-display.test.ts: メッシュ表示方法、比較設定、ジョイント表示設定の初期値、切替、複製、同値更新抑止、購読通知、reset のテスト
