...
- display.ts: ルーム共有メッシュ表示方法、メッシュ比較設定、ジョイント表示設定、モーション軌跡表示設定を複製保持し、同値更新を抑止する `useDisplayStore` を提供する
...
display ストアは `MeshDisplayMode`、`MeshCompare`、`JointDisplay`、`MotionTrail` を保持し、初期値と reset はそれぞれの shared 既定値の複製、同値の set は state と購読通知を変えない。比較設定、ジョイント表示設定、軌跡表示設定の setter、welcome 受信値は必要な参照まで複製して保持する。
