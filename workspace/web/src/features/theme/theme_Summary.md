...
- ColorWheel.tsx: 色相リングと彩度明度四角を操作し、`WHEEL_SIZE` を `--wheel-size` として渡す props 駆動の React 部品
- theme-palette.ts: `THEME_PALETTE_COLUMNS` による 9 列 3 行の固定パレットとスウォッチ上の印の色
- theme-labels.ts: 表示色設定パネルの日本語ラベルと aria-label 生成関数
- ColorPicker.tsx: カラーホイール、`THEME_PALETTE_COLUMNS` を `--theme-palette-columns` として渡す固定パレット、`HEX_INPUT_MAX_LENGTH` による上限付きの16 進入力をまとめた制御部品
...
- color-wheel.test.ts: ホイール座標、色決定、RGBA 画像、React 部品の DOM・CSS サイズ変数とポインター操作
...
- theme-styles.test.ts: 表示色設定 CSS のレイアウト、CSS 変数のフォールバックと TS 定数の一致、属性状態、固定色禁止
