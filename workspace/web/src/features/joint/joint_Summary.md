...
表示設定は display ストアの `jointDisplay`(ルーム共有・設計書 §13.5)。可視化は `VIEWER_OVERLAY_KEY` を持つためアウトライナ・部位パス・選択・比較の対象にならない。配置は ReviewPage(107)。HUD への設置は `features/viewer/ViewerHud.tsx`。スタイルは viewer.css の `.hud-display` を再利用する。scene は compare の `useModelScenesStore` から取得し、描画更新は React Three Fiber の `useFrame` で行う。
