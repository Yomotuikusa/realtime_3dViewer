...
OBJLoader / FBXLoader が三角形分割した多角形について、元の輪郭辺を判定する頂点属性を geometry に付ける。OBJ の Mesh オブジェクトごとに面の頂点数を読み取り、FBX は Geometry の PolygonVertexIndex と Model/Geometry の接続を読み取り、専用ローダーで属性付与まで結び付ける。
...
three の `OBJLoader` が返す非インデックス Mesh geometry と OBJ の file 順の面情報を結び付ける。`FBXLoader` が返す Model ID 付き Mesh には、FBX の `Connections` による Model/Geometry 対応と `PolygonVertexIndex` の面情報を結び付ける。描画用の属性利用やビューアの表示切替は `viewer/` の後続タスクが担当する。
