...
## ファイル一覧と役割
- StrokeLines.tsx: annotation ストアの透過表示設定と view-settings の線幅・透過線比を購読し、受け取った `Stroke[]` を通常線と必要な深度テスト無効の透過線として drei `Line` で描画する表示コンポーネント
- stroke-overlay.ts: `Stroke` から通常線・透過線の描画 spec を組み立てる定数、オプション型、純粋関数
...
## 公開インターフェイス
- StrokeLines.tsx: `StrokeLines({ strokes, opacity? })`（annotation ストアの `overlay` と view-settings の線幅・透過線比を購読）
- stroke-overlay.ts: `BASE_LINE_WIDTH`、`OVERLAY_LINE_WIDTH_RATIO`、`OVERLAY_LINE_WIDTH`、`OVERLAY_OPACITY_RATIO`、`StrokeLineSpec`、`StrokeLineOptions`、`strokeLineSpecs`
...
双方で同じ `overlay` 設定を購読し、view-settings の `strokeWidth` と `overlayOpacityRatio` を通常線と透過線へ適用する。通常線に加えて必要な場合だけ深度テスト無効の透過線を重ねる。
