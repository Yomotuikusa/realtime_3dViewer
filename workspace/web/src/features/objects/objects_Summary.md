...
- CompareControls.tsx: 基準・対象の版と比較しきい値を表示・更新し、比較中も基準を表示するチェックボックスを備え、`mesh:compare` を WebSocket 共有する
- DeleteObjectDialog.tsx: オブジェクト削除の確認文、危険操作ボタン、キャンセルを含む alertdialog を表示する
- objects-labels.ts: オブジェクト見出し、可視性、ファイル追加、削除、比較の文言（比較中も基準を表示するラベルを含む）と表示用 helper
