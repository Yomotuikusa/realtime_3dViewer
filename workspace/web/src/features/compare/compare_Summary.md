...
- overlay.ts: 符号付き距離を指定された赤・青・透明の頂点色で比較対象 Mesh に重ね描きし、再利用・破棄する関数を提供する。既定の比較色は viewer color defaults から取得する
