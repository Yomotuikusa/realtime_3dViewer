...
- ViewerHud.tsx: ペン／コメントの toggle ボタンとペン道具を左上に、最初から展開した半透明カメラパネルを右上に表示し、CameraMenu、LightGizmo、Follow 中の参加者色フレームと左端に接して面の全高を占め左下だけ角丸の参加者色の縦帯・不透明な面の上辺タブ、描画基準、操作ヒントを各ストアと keymap に接続する。メニュー内の Escape はメニューだけを閉じる
