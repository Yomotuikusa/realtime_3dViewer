...
- src/app/realtime-dispatch.ts: `ServerMessage` を session / presence ストアへ振り分ける入口。`welcome`、presence 更新、`error` を扱う
...
`dispatchServerMessage` は `welcome` で session の self ID/色と presence の参加者一覧を確定し、
`user:joined` / `user:left` / `camera` を presence ストアへ、`error` を `CODE: message` として保存する。
