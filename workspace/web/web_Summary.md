...
## 目的
REST API の fetch クライアント、ブラウザ履歴による入口ルーティング、モデルのアップロード画面を提供する。
...
- src/app/ReviewPage.tsx: projectId を表示するレビュー画面のプレースホルダ
...
## 他機能との関係
`@shared/api` の API エラー・入力型・アップロード拡張子と、`@shared/types` の Project/Comment スキーマを利用する。後続の viewer/comment 機能は `getProject`、`modelUrl`、コメント API、ReviewPage のプレースホルダを引き継ぐ。
