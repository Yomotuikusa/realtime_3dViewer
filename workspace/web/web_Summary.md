...
- src/app/realtime-dispatch.ts: `ServerMessage` を各機能ストアへ振り分ける入口。現在は `welcome` と `error` を扱い、後続機能が case を追加する
- src/app/useRealtime.ts: 名前決定後の `WsClient` 接続と、open ごとの `join` 送信
- src/app/UploadPage.tsx: プロジェクト名・`.glb`/`.gltf` のアップロード画面
- src/app/ReviewPage.tsx: プロジェクト取得、入室ダイアログ、接続状態、ロード状態・エラーカード、ビューアとサイドパネルのレイアウト
...
- tests/realtime-dispatch.test.ts: welcome/error の session 反映、未対応イベント、reset のテスト
...
`dispatchServerMessage` は `welcome` で session の self ID/色を確定し、`error` を `CODE: message`
として保存する。新しい受信型はこの関数の `switch` に case を追加して各機能ストアへ反映する。
