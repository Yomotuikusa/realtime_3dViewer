...
- `src/routes/projects.ts`: project JSON の取得と、モデル本体の配信。拡張子に応じた
  Content-Type と immutable キャッシュヘッダを設定する。
...
- `projectRoutes`: `GET /api/projects/:projectId` と
  `GET /api/projects/:projectId/versions/:versionId/model` を提供する。
