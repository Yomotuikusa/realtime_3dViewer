...
| 区分 | 状態 | 保持場所 | 経路 |
| --- | --- | --- | --- |
| 共有(ルーム) | 版の表示・非表示、版内オブジェクト(部位)の表示・非表示、メッシュ表示方法、メッシュ比較設定、ライトの向き、注釈の線 | `RoomHub` の `Room`(サーバメモリ。ルームが空になったら破棄) | `ClientMessage` で送信 → サーバが状態を更新し他者へ中継 → 後から入った人には `welcome` で全置換 |
| ローカル(端末) | アウトライナの選択・展開状態、ドック幅、自分のカメラ(他者へは presence として中継)、コメント再現の線、注釈の透過表示、**UI テーマ(ライト / ダーク / OS 追従)と 3D ビューの色設定** | zustand ストア / localStorage | WS には流さない |
