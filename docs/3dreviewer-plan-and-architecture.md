...
### 13.5 表示状態の共有原則(2026-09-12 確定)

**「3D ビューに何がどう見えるか」を決める状態は、すべてルーム(プロジェクト)で共有する。**
同じ URL を開いている全員が同じものを見ていることが中核体験(§3)であり、
「自分だけ非表示」「自分だけワイヤフレーム」のようなローカル差分は作らない。

| 区分 | 状態 | 保持場所 | 経路 |
| --- | --- | --- | --- |
| 共有(ルーム) | 版の表示・非表示、版内オブジェクト(部位)の表示・非表示、メッシュ表示方法、メッシュ比較設定、ライトの向きと明るさ、注釈の線 | `RoomHub` の `Room`(サーバメモリ。ルームが空になったら破棄) | `ClientMessage` で送信 → サーバが状態を更新し他者へ中継 → 後から入った人には `welcome` で全置換 |
| ローカル(端末) | アウトライナの選択・展開状態、ドック幅、自分のカメラ(他者へは presence として中継)、コメント再現の線、注釈の透過表示、**UI テーマ(ライト / ダーク / OS 追従)と 3D ビューの色設定**、**表示と操作の設定**(線幅・重ね描きの不透明度・操作感度・ジョイントの大きさ・アウトライナの密度。D40) | zustand ストア / localStorage | WS には流さない |

テーマと色設定をローカルに置くのは、この原則の**意図的な例外**である(2026-09-13 確定)。共有原則が守ろうとしているのは「同じものを指して話せること」であり、**何が見えているか**(どの版・どの部位・どの表示方法)は共有しなければ壊れるが、**それがどんな色で見えているか**は指示語の通じ方を変えない。一方で見やすい配色は視力・ディスプレイ・環境光によって人ごとに違うため、共有すると他者の見やすさを一方的に書き換えてしまう。ショートカットキー(`features/shortcuts`)と同じ「端末の都合」として扱い、`resetReviewStores()` の対象からも外す。ただし**色そのものを会話の内容にする機能**(比較の色分けの凡例など)を作るときは、凡例の意味づけを共有側へ置くか、この判断を見直すこと。

新しい「表示に関わる状態」を追加するときは、次の 4 点を**同じ機能として**起票する。
1. `shared/src/protocol.ts` に client → server と server → client のメッセージ、`welcome` の復元フィールド
2. `server/src/realtime/` でルーム状態の更新・中継・`welcome` への搭載
3. `web/src/app/realtime-dispatch.ts` で受信をストアへ反映(welcome は全置換)
4. 送信側 UI はローカルのストアを更新してから `send` する(サーバは送信者に返さない)

**オブジェクトの識別**: three.js の `Object3D.uuid` は読み込みごと・クライアントごとに変わるため、
ルーム共有の鍵には使えない。版内のオブジェクトは、版の scene ルートからの
「子インデックスのパス」`ObjectPath`(例: `"0/2/1"`。ビューアが後付けした重ね描きは数えない)で識別する。
ルート自身は版そのもの(`versionId`)として扱う。
...
- 環境変数: `PORT`(既定 3000)、`DATA_DIR`(既定 `./data`)、`MAX_UPLOAD_BYTES`(既定 100MB)、`WEB_DIST_DIR`(既定 `./web/dist`)、
  `WS_HEARTBEAT_INTERVAL_MS`(既定 30000。ping の間隔。0 で無効。140)、`MAX_UPLOAD_FILES`(既定 20。1 リクエストのモデルファイル数。141)
