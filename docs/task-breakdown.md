...
| D37 | 対象環境 | デスクトップのみ。**幅 1024px 以上**を保証し、それ未満の崩れは許容する(設計書 §8 の仮説にモバイルは無い)。ライトテーマのみ。Web フォントは読まない(`system-ui` 系スタック)。動きは opacity / 小さな translate のみ、`prefers-reduced-motion` で無効化。`docs/DESIGN_SKILL.md` §8(レスポンシブ)の要求はこの決定で意図的に外す |
...
レスポンシブ / モバイル(D37)、ダークテーマ、アニメーションの作り込み、空状態のイラスト、
