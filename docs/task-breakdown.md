...
| D40 | **端末ローカルの「表示と操作」設定**(2026-09-16 確定) | D39 の例外(テーマ・色設定)を「ディスプレイ・入力機器で最適値が違う値」へ広げる。注釈の線幅・透過線の比、選択 / ワイヤー重ね描きの不透明度、dolly / ライト回転の感度(倍率)、ジョイント球の倍率とピック半径、アウトライナの行高と字下げ。`web/src/features/view-settings/`(`useViewSettingsStore`、localStorage `3dreviewer:view-settings`、設定ダイアログ第 3 タブ「表示と操作」)。**全キーの既定値・範囲・UI は 144 が定義し、145〜147 は消費するだけ**(D28 の流儀)。`resetReviewStores()` の対象外。センサーサイズ(焦点距離の意味を変える)と simplify 許容誤差(送信データを変える)はローカルにしない |
| D41 | **ライトの明るさはルーム共有**(2026-09-16 確定) | 明るさは「何が見えているか」を変えるので D39 どおり 4 点セットで共有する。値は既定の 3 灯の強さに掛ける倍率(`MIN_LIGHT_BRIGHTNESS` 0.25 〜 `MAX_LIGHT_BRIGHTNESS` 4、既定 1)。メッセージは向きと別の `light:brightness`、welcome は `lightBrightness?`。UI はライトギズモ直下のスライダー、送信は向きと同じ 50ms throttle。148(shared + server)/ 149(web) |
| D42 | カメラの near / far | `<Canvas>` の既定(0.1 / 2000)ではなく、camera ストアの `modelSize` から `near = size × 0.01`、`far = size × 200`(far / near = 2×10⁴、three 既定と同じ深度精度)。`FocalLengthRig` と同型の `ClipPlanesRig`。142 |
| D43 | 運用上限の環境変数 | `WS_HEARTBEAT_INTERVAL_MS`(既定 30000、0 で無効。ping/pong の `isAlive` 方式で無応答は `terminate()` → 既存の `finalize` → `user:left`)、`MAX_UPLOAD_FILES`(既定 20、超過は 400 `VALIDATION`)。web/dist の既定パスは `config.ts` の `DEFAULT_WEB_DIST_DIR` に一本化。140 / 141 |
| D44 | 定数の重複の扱い | shared の上限(`MAX_PROJECT_NAME_LENGTH` 等)は UI の `maxLength` が import する(`JoinDialog` の形)。CSS に書かざるを得ない寸法(初回描画のフォールバック)は `.tsx` が同名の custom property を inline で渡し、CSS 側は `var(--x, フォールバック)`、フォールバック = TS 定数をテストで機械検証する。136〜138 |
...
## 6. 履歴に同じ id が 2 回出てくる理由(2026-09-16、136〜142)

**`git log` で「task 136」〜「task 142」が別内容で 2 組見つかるのは、リポジトリの壊れではない。**
2 台の PC で同時に起票が走り、どちらも当時の最大 id(135)の続きとして 136 から採番したためである。

- PC-A(メッシュ比較の系列): 136 しきい値下限 0 / 137 面単位の着色 / 138 `baseVisible` /
  139 基準の描画抑止 / 140 0.1‰ 刻み / 141 目盛スライダー / 142 フラグメント判定
- PC-B(web・server・shared の系列): 136 ストローク上限の定数化 〜 150 表示設定の HUD 移設

PC-B を先に push したため、後から push した PC-A 側のタスク md を **151〜157 に振り直した**
(ファイル名・`id:`・`depends_on:`・本文中の相互参照をまとめて +15)。よって:

- **`tasks/` の id は一意である。**151〜157 が旧 136〜142(メッシュ比較)にあたる
- **コミットメッセージだけが当時の番号のまま**で、振り直していない。履歴を書き換えると
  既に push 済みの 53 コミットが壊れるため、意図的に残した
- 履歴を id で辿るときは、`tasks/` のファイル名(内容)と突き合わせること。
  番号だけで一致を判断しない

マージは 2026-09-16 に `git merge origin/main` で解消済み。競合したのは
`shared_Summary.md` / `objects_Summary.md` / `viewer_Summary.md` の 3 件だけで、
いずれも箇条書きの索引なので両側の記述を合成して解決した。コードの競合は無かった。

**同じことを繰り返さないために**: 起票の直前に `git fetch && git log --oneline origin/main`
で他機の採番を確認し、次の id はローカルとリモート両方の最大値の次を取る。
